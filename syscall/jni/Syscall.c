...
int main(int argc, char *argv[]) {
    ...
    // syscall read
    local[0].iov_base = &DestBuf;
    local[0].iov_len = sizeof(DestBuf);

    remote[0].iov_base = (void *)readAddr;
    remote[0].iov_len = sizeof(readAddr);
    ...
    // syscall read again
    syscall(__NR_process_vm_readv, pid, local, 1, remote, 1, 0);
